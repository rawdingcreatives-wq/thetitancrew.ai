// @ts-nocheck
/**
 * TitanCrew Â· Morning Briefing Email Template
 *
 * Sent daily at 6am local time by the Foreman Agent.
 * This is the #1 "aha moment" for new users â it proves the AI crew
...
<head>
  ...
  <title>Your Morning Briefing â ${data.businessName}</title>
</head>
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0F172A;padding:20px 0;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#1E293B;border-radius:12px;overflow:hidden;border:1px solid rgba(255,255,255,0.1);">
        ...
        <tr>
          <td style="background:linear-gradient(135deg,#1A2744 0%,#FF6B00 150%);padding:32px 32px 24px;">
            ...
            <div style="margin-top:20px;font-size:22px;font-weight:700;color:#FFFFFF;">
              Good morning, ${data.ownerFirstName} ð
            </div>
        ...
        <tr>
          <td style="padding:24px 32px 0;">
            <div style="font-size:15px;font-weight:700;color:#FFFFFF;margin-bottom:12px;">ð¤ Crew Activity Overnight</div>
            <table width="100%" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td style="padding:24px 32px 0;">
            <div style="font-size:15px;font-weight:700;color:#FFFFFF;margin-bottom:12px;">ð Today's Schedule</div>
          ...
          <td style="padding:20px 32px;border-top:1px solid rgba(255,255,255,0.05);">
            <table width="100%"><tr>
              <td style="font-size:11px;color:rgba(255,255,255,0.3);">
                ${data.businessName} Â· Powered by TitanCrew AI
              </td>
...
export function buildMorningBriefingText(data: MorningBriefingData): string {
  const lines: string[] = [
    ...
    "",
    `ð TODAY: ${data.jobsToday} jobs | THIS WEEK: ${data.jobsThisWeek} jobs | REVENUE: $${data.revenueThisWeek.toLocaleString()}`,
    `ð° OUTSTANDING: ${data.outstandingInvoices} invoices ($${data.outstandingAmount.toLocaleString()})`,
    "",
  ];

  if (data.alerts.length) {
    lines.push("â ï¸ ALERTS:");
    data.alerts.forEach(a => lines.push(`  â¢ ${a.message}`));
    lines.push("");
  }

  if (data.agentActions.length) {
    lines.push("ð¤ CREW ACTIVITY OVERNIGHT:");
    data.agentActions.forEach(a => lines.push(`  ${a.icon} ${a.agent}: ${a.action} â ${a.detail}`));
    lines.push("");
  }

  if (data.todaySchedule.length) {
    lines.push("ð TODAY'S SCHEDULE:");
    data.todaySchedule.forEach(j => lines.push(`  ${j.time} â ${j.customer} (${j.service}) â ${j.tech} â $${j.amount}`));
    lines.push("");
  }

  if (data.pendingApprovals > 0) {
    lines.push(`ð ${data.pendingApprovals} action(s) waiting for your approval: ${data.approvalUrl}`);
    lines.push("");
  }
